...
function renderQueue() { clear(elements.queueContent); const queue = state.player?.queue || []; elements.queueContent.append(renderSongs(queue, "当前没有播放队列")); updateQueueStatus(state.player); }
...
elements.mode.addEventListener("click", async () => { const currentIndex = Math.max(0, playbackModes.findIndex((item) => item.value === state.player?.playMode)); const nextMode = playbackModes[(currentIndex + 1) % playbackModes.length]; const result = await message(MessageType.SET_PLAY_MODE, { mode: nextMode.value }); if (!result?.ok) { showNotice(result?.message || "无法切换播放模式"); return; } renderPlayer(result.state); });
elements.queue.addEventListener("click", () => { renderQueue(); elements.queuePanel.hidden = false; });
elements.closeQueue.addEventListener("click", () => { elements.queuePanel.hidden = true; });
document.addEventListener("keydown", (event) => { if (event.key === "Escape" && !elements.queuePanel.hidden) elements.queuePanel.hidden = true; });
...
document.querySelector('[data-control="previous"]').innerHTML = icon("previous"); document.querySelector('[data-control="next"]').innerHTML = icon("next"); elements.queue.innerHTML = icon("queue"); elements.closeQueue.innerHTML = icon("back");
