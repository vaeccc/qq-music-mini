...
const state = { tab: "liked", library: null, searchKeyword: "", searchResults: [], detail: null, player: null, poll: null };
const elements = {
...
function renderSearch() {
  ...
  elements.content.append(form, renderSongs(state.searchResults, "输入关键词搜索歌曲"));
}
...
async function startLogin() { showNotice(); elements.qr.hidden = true; const result = await message(MessageType.LOGIN_START); if (!result.ok) { showNotice(result.message || "登录失败，请重新尝试"); return; } elements.qrImage.src = result.imageUrl; elements.qr.hidden = false; elements.qrStatus.textContent = "请使用 QQ 扫码登录"; clearInterval(state.poll); state.poll = setInterval(checkLogin, 2500); }
async function checkLogin() { const result = await message(MessageType.LOGIN_STATUS); if (!result.ok) { clearInterval(state.poll); showNotice(result.message || "登录失败，请重新尝试"); return; } if (result.status === "waiting") return; if (result.status === "scanned") { elements.qrStatus.textContent = "已扫码，请在手机上确认"; return; } if (result.status === "expired") { clearInterval(state.poll); elements.qrStatus.textContent = "二维码已失效，请重新获取"; return; } if (result.status === "done") { clearInterval(state.poll); elements.login.hidden = true; elements.music.hidden = false; await loadLibrary().catch((error) => showNotice(error.message || "网络请求失败")); } }
...
(async () => { const player = await message(MessageType.GET_PLAYER_STATE); renderPlayer(player.state); try { await loadLibrary(); elements.music.hidden = false; } catch (error) { elements.login.hidden = false; if (error.error !== "LOGIN_EXPIRED") showNotice(error.message || "网络请求失败"); } })();
